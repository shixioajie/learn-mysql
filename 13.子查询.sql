...
SELECT EXISTS(SELECT employee_id FROM employees WHERE `salary` <= 0);#0
...
WHERE EXISTS 
  (SELECT 
    em.`last_name` 
  FROM `employees` AS em WHERE em.`department_id`=dep.`department_id`);
...
FROM
  employees AS em1 
WHERE 
em1.`salary` > (SELECT AVG(salary)FROM `employees` )
ORDER BY em1.`salary` 
;
...
#①查询姓名为king的员工编号
SELECT last_name,employee_id FROM `employees` WHERE last_name = 'K_ing';
